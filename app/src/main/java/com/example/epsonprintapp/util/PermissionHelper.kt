 ...
 *
 * minSdk = 33 (Android 13): no hay ramas por versión — siempre aplican
 * los permisos granulares de Tiramisu.
 *
 * Permisos por categoría:
 * - RED: ACCESS_FINE/COARSE_LOCATION — Android exige ubicación para el
 *   discovery mDNS (NsdManager); sin él, discoverServices() falla en silencio.
 * - ALMACENAMIENTO: READ_MEDIA_IMAGES — seleccionar imágenes para imprimir.
 *   (Los PDF se abren vía Storage Access Framework, sin permiso.)
 * - NOTIFICACIONES: POST_NOTIFICATIONS — avisos de impresión/escaneo.
 */
object PermissionHelper {

    val NETWORK_PERMISSIONS: Array<String> = arrayOf(
    ...
    val NOTIFICATION_PERMISSIONS: Array<String> = arrayOf(
        Manifest.permission.POST_NOTIFICATIONS
    )

    /** Todos los permisos que la app necesita, agrupados para la solicitud inicial. */
    fun getAllRequiredPermissions(): Array<String> =
        (NETWORK_PERMISSIONS + STORAGE_PERMISSIONS + NOTIFICATION_PERMISSIONS)
            .distinct()
            .toTypedArray()

    /** Retorna los permisos que aún no han sido otorgados. */
    fun getMissingPermissions(context: Context): Array<String> =
        getAllRequiredPermissions().filter { permission ->
